...
const Skill = ({ name, href, children, size = 'md' }: SkillProps) => {
    ...
    const content = (
        <div className={cn(
            "flex items-center bg-card border border-border border-dashed text-foreground/90 hover:text-foreground hover:border-neutral-500/40 hover:bg-accent transition-all cursor-default group shadow-sm",
            isSm
            ...
            {children && (
                <span className={cn(
                    "text-muted-foreground group-hover:text-foreground transition-colors flex items-center justify-center shrink-0",
                    isSm
                        ? "[&>svg]:w-3 [&>svg]:h-3"
